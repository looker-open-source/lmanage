...
class CaptureDashboards():
    ...
    def get_all_dashboards(self) -> dict:
        ...
        scrub_dashboards = {dash.id: dash.folder.id for dash in all_dashboards if not dash.folder.is_personal and not dash.folder.is_embed and dash.folder.id!= 'lookml'}
    ...
    def get_dashboard_lookml(self, all_dashboards: dict) -> list:
        ...
        for dash_id in tqdm(all_dashboards, desc = "Dashboard Capture", unit=" dashboards", colour="#2c8558"):
            ...
            if "::" in dash_id:
                ...
                dash_board_lookml = yaml.safe_load(lookml.lookml)

                captured_dashboard = DashboardObject(
                    legacy_folder_id=all_dashboards.get(dash_id),
                    lookml=dash_board_lookml,
                    dashboard_id=dash_id)
                response.append(captured_dashboard.__dict__)
        return response
