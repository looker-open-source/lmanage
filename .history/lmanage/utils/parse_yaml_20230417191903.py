...
class Yaml:
    ...
    def get_permission_metadata(self):
        ...
        for objects in self.parsed_yaml:
            print(objects.name)
            if 'permissions' in list(objects.__dict__.keys()):
    ...
    def get_model_set_metadata(self):
        response = []
        for objects in self.parsed_yaml:
            if 'models' in list(objects.__dict__.keys()):
                response.append(objects)
        return response
    ...
    def get_role_metadata(self):
        response = []
        for objects in self.parsed_yaml:
            if 'permission_set' and 'model_set' in list(objects.__dict__.keys()):
                response.append(objects)
        response = [r for r in response if r.get('name') != 'Admin']
        return response
    ...
    def get_user_attribute_metadata(self):
        response = []
        for objects in self.parsed_yaml:
            if 'hidden_value' in list(objects.__dict__.keys()):
                response.append(objects)
        return response
    ...
    def get_look_metadata(self):
        response = []
        for objects in self.parsed_yaml:
            if 'query_obj' in list(objects.__dict__.keys()):
                response.append(objects)
        return response
    ...
    def get_dash_metadata(self):
        response = []
        for objects in self.parsed_yaml:
            if 'lookml' in list(objects.__dict__.keys()):
                response.append(objects)
        return response
