...
def test_create_folder(mocker):
    ...
    sf_data = fake_methods_data.MockCreateFolder(name='frankie_fish',parent_id='123')
    mocker.patch.object(sdk, "create_folder")
    sdk.create_folder.return_value = sf_data

    test = cf.CreateInstanceFolders(sdk=sdk, folder_metadata='5555').create_folder(folder_name='test', parent_id='123')
