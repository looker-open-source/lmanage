...
def test_get_role_permissionset_metadata():
    # Tests how the input of the yaml file
    test = rc.get_role_creation_metadata(
        parsed_yaml=input_data, metadata_type='permission_sets')
    assert len(test[0]) == 2
    assert test[0].get('role_name') == 'developer'
    assert isinstance(test, list)


def test_get_role_modelset_metadata():
    # Tests how the input of the yaml file
    test = rc.get_role_creation_metadata(
        parsed_yaml=input_data, metadata_type='model_sets')
    assert len(test[0]) == 6
