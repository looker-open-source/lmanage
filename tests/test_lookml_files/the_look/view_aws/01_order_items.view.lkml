view: order_items {
  ...
  measure: count {
    type: count_distinct
    sql: ${id} ;;
    drill_fields: [detail*, total_gross_margin_percentage, total_gross_margin, -created_date]
  }
  ...
  dimension: order_id {
    ...
    action: {
      ...
      param: {
        name: "user_dash_link"
        value: "/dashboards/24?Email={{ users.email._value}}"
      }
  ...
  dimension: created_reporting_week {
    type: string
    sql: ${created_year} || ' W' || ${created_week_of_year} ;;
  }

  dimension: end_of_current_month  {
    hidden: yes
    type: date
    convert_tz: no
    sql: DATEADD(day,-1,DATEADD(month,1,DATE_TRUNC('month',GETDATE()))) ;;
  }
